...
function criarNovaLista(event) {
    ...
    //pega o id das listas
    const procurarId = document.querySelectorAll('.task-column')
    let idsListas = [];

    procurarId.forEach(lista => {
        const pegarId = Number(lista.getAttribute('data-id'))
        idsListas.push(pegarId);
        console.log(pegarId);
        
    })
    localStorage.setItem('id_listas', JSON.stringify(idsListas));
}
